...
namespace CMath {
    ...
    inline double find_surd_expression(const double& d) {
        ...
        for (int i = 0; i < d; i++) {
            for (int j = 2; j < d; j++) {
                if (is_perfect_square(j) && is_perfect_square(i)) {
                    result = i * j;
                    if (result == d) {
        ...
        outer_surd = std::sqrt(squareOSrd);
        std::cout << "Values of x in surd form, given that f(x) = 0: "<< std::setprecision(3) << outer_surd << "+ sqrt(" << multiples.at(1) <<")\n";
        std::cout << "Values of x in surd form, given that f(x) = 0: "<< std::setprecision(3) << outer_surd << "- sqrt(" << multiples.at(1) <<")\n";
        return std::nan("Done");
    }
}
